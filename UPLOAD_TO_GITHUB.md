# Загрузка Dadway VPN v8.0.0 в GitHub
...
Скопируйте в `D:\git\DadwayVPN` всё содержимое папки версии 8.0.0 с заменой файлов, но не удаляйте папку `.git`. Затем:

```powershell
cd D:\git\DadwayVPN
git status
git add --all
git commit -m "Dadway VPN v8.0.0"
...
После загрузки откройте **GitHub → Actions → Build Dadway VPN v8.0.0 APK**. Скачайте артефакт `DadwayVPN-v8.0.0-apk`.
