# Dadway VPN 8.5.9 — инструкция пользователя
...
> Иллюстрации ниже повторяют элементы интерфейса версии 8.5.9. Расположение может немного отличаться в зависимости от размера экрана и выбранной темы.
...
Миграция, добавленная в 8.5.6, один раз удаляет ранее сохранённые подписки, их кэши и выбранный сервер. Это обеспечивает действительно пустой список после установки обновления. Подписки, добавленные после завершения миграции, при следующих запусках не удаляются.
...
Зелёный статус **Готово** означает, что туннель запущен. После первого подключения приложение автоматически проверит внешний IP и скорость. Поле **Пинг** показывает медиану трёх TCP-измерений непосредственно до выбранного VPN-сервера, а не задержку стороннего сайта проверки.
